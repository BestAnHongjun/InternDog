...
def get_door_info(cli=True):
    if cli:
        room = input("------\n现在，由您来充当底层程序，您可以决定InternDog看到的门牌信息。\nInternDog看到的门牌号是？（输入一个三位数或四位数，比如605，1104，1501……）\n[你的输入] >>> ")
        while True:
...
def turn_left(cli=True):
    if cli:
        message = "[底层程序消息]已向左转向。"
        return message


def turn_right(cli=True):
    if cli:
        message = "[底层程序消息]已向左转向。"
        return message
